...
import (
	...
	"github.com/pulse-downloader/pulse/internal/testutil"
)
...
func TestConcurrentDownloader_ResumePartialDownload(t *testing.T) {
	...
	tmpDir, cleanup, _ := testutil.TempDir("pulse-resume-test")
	defer cleanup()

	destPath := filepath.Join(tmpDir, "resume_test.bin")
	workingPath := destPath + types.IncompleteSuffix

	// Create partial .pulse file (simulate interrupted download)
	partialSize := int64(100 * types.KB)
	_, err := testutil.CreateTestFile(tmpDir, "resume_test.bin.pulse", partialSize, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Verify final file exists (not .pulse)
	if testutil.FileExists(workingPath) {
		t.Error(".pulse file should be removed after completion")
	}
...
func TestConcurrentDownloader_SmallFile(t *testing.T) {
	...
	tmpDir, cleanup, err := testutil.TempDir("pulse-download-test")
	if err != nil {
		t.Fatal(err)
	}
	defer cleanup()
	...
	pulseFile := destPath + types.IncompleteSuffix
	if testutil.FileExists(pulseFile) {
		t.Error(".pulse file should be removed after successful download")
	}
}
...
func TestConcurrentDownloader_MediumFile(t *testing.T) {
	...
	tmpDir, cleanup, _ := testutil.TempDir("pulse-download-test")
	defer cleanup()
...
func TestConcurrentDownloader_Cancellation(t *testing.T) {
	...
	tmpDir, cleanup, _ := testutil.TempDir("pulse-cancel-test")
	defer cleanup()
...
func TestConcurrentDownloader_ProgressTracking(t *testing.T) {
	...
	tmpDir, cleanup, _ := testutil.TempDir("pulse-progress-test")
	defer cleanup()
...
func TestConcurrentDownloader_RetryOnFailure(t *testing.T) {
	...
	tmpDir, cleanup, _ := testutil.TempDir("pulse-retry-test")
	defer cleanup()
...
func TestConcurrentDownloader_FailOnNthRequest(t *testing.T) {
	...
	tmpDir, cleanup, _ := testutil.TempDir("pulse-failnth-test")
	defer cleanup()
...
func TestConcurrentDownloader_WithLatency(t *testing.T) {
	...
	tmpDir, cleanup, _ := testutil.TempDir("pulse-latency-test")
	defer cleanup()
...
func TestConcurrentDownloader_SlowDownload(t *testing.T) {
	...
	tmpDir, cleanup, _ := testutil.TempDir("pulse-slow-test")
	defer cleanup()
...
func TestConcurrentDownloader_RespectServerConnectionLimit(t *testing.T) {
	...
	tmpDir, cleanup, _ := testutil.TempDir("pulse-connlimit-test")
	defer cleanup()
...
func TestConcurrentDownloader_ContentIntegrity(t *testing.T) {
	...
	tmpDir, cleanup, _ := testutil.TempDir("pulse-integrity-test")
	defer cleanup()
