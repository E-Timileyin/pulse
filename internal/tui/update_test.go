...
func TestGenerateUniqueFilename(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "pulse-tui-test-*")
	if err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		...
		{
			name:          "Conflict with .pulse file (paused download)",
			existingFiles: []string{"file.txt.pulse"},
			inputFilename: "file.txt",
			want:          "file(1).txt",
		},
		{
			name:          "Conflict with both final and .pulse file",
			existingFiles: []string{"file.txt", "file(1).txt.pulse"},
			inputFilename: "file.txt",
			want:          "file(2).txt",
		},
		{
			name:          "Multiple .pulse conflicts",
			existingFiles: []string{"1GB.bin.pulse", "1GB(1).bin.pulse"},
		...
		{
			name:           "Combined: .pulse file and active download",
			existingFiles:  []string{"file.txt.pulse"},
			activeDownload: "file(1).txt",
			inputFilename:  "file.txt",
			want:           "file(2).txt",
...
func TestGenerateUniqueFilename_IncompleteSuffixConstant(t *testing.T) {
	...
	if types.IncompleteSuffix != ".pulse" {
		t.Errorf("IncompleteSuffix = %q, want .pulse", types.IncompleteSuffix)
	}
}
