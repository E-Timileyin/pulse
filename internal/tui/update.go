...
import (
	...
	"github.com/pulse-downloader/pulse/internal/version"
				...
				if m.list.FilterState() == list.Filtering {
					...
					if realIdx != -1 {
						...
						if !dl.done && dl.Destination != "" {
							// Delete the .pulse partial file with retries
							// (worker may still hold file briefly after Cancel on Windows)
							pulseFile := dl.Destination + types.IncompleteSuffix
							for i := 0; i < 5; i++ {
								if err := os.Remove(pulseFile); err == nil {
									break
								}
...
func (m *RootModel) generateUniqueFilename(dir, filename string) string {
	...
	// Check if file exists on disk (including incomplete .pulse files)
	existsOnDisk := func(name string) bool {
		path := filepath.Join(dir, name)
		if _, err := os.Stat(path); !os.IsNotExist(err) {
			return true
		}
		// Also check for incomplete download file (.pulse extension)
